...
#define BARGRAPH_X_SIZE 172
#define BARGRAPH_Y_SIZE 122
...
static void _DrawDiagramAt(int xPos, int yPos, int * py, int xBlend) {
  ...
  //
  if (hMem == 0) {
    hMem = GUI_MEMDEV_Create(xPos - 7, yPos, BARGRAPH_X_SIZE + 7 + 1, BARGRAPH_Y_SIZE + 1);
    if (hMem == 0) {
...
#endif
  GUI_FillRoundedRect(xPos, yPos, xPos + BARGRAPH_X_SIZE, yPos + BARGRAPH_Y_SIZE, 4);
  //
  // Draw grid lines
  //
#if (GUI_USE_ARGB)
...
#endif
  for (i = 0; i < 12; i++) {
    GUI_DrawHLine(yPos + 6 + i * 10, xPos + 2, xPos + BARGRAPH_X_SIZE - 2);
  }
  //
  // Draw bars
  //
  for (i = 0; i < 10; i++) {
    ...
    ySizeBar = *(py + i);
    GUI_DrawBitmapMag(_apBmBar[IndexBmBar], xPos + 8 + i * 16, yPos + BARGRAPH_Y_SIZE - ySizeBar - 6, 1, ySizeBar);
  }
  //
  // Draw alpha effect
  //
  ...
  GUI_FillRect(xPos + 4, yPos + 1, xPos + xBlend, yPos + BARGRAPH_Y_SIZE - 1);
#if (GUI_USE_ARGB)
  GUI_SetAlpha(0xFF);
#else
  GUI_SetAlpha(0);
...
#endif
  GUI_DrawRoundedRect(xPos, yPos, xPos + BARGRAPH_X_SIZE, yPos + BARGRAPH_Y_SIZE, 4);
  //
  // Label
  //
...
//  GUI_Init();
  ...
  xSize       = LCD_GetXSize();
  ySize       = LCD_GetYSize();
  xPos        = (xSize - BARGRAPH_X_SIZE) / 2;
  yPosText    = (ySize - BARGRAPH_Y_SIZE - 24) / 3;
